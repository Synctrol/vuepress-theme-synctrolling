...
| `src/compiler/release/list-model.ts` | Date-desc sort, `index.enabled`, pagination slices, tile models, placeholder selection |
| `src/compiler/release/detail-model.ts` | Detail section order, book identity, type-specific body payload, markdown-only path |
| `src/compiler/release/jsonld-album.ts` | Build `MusicAlbum` + `MusicRecording` JSON-LD for album books only |
...
| `tests/compiler/release/*.test.ts` | List/detail/JSON-LD model tests |
...
// Plan 04 (assets) — canonical types in src/shared/asset-types.ts
// ResolvedAsset fields: kind, sourcePath, assetPath, publicPath, absoluteUrl, contentHash
// (use publicPath, not publicUrl). Lookup via AssetManifest.contentPublicPaths /
// globalPublicPaths or client resolveContentAsset from vuepress-theme-synctrolling/client.
export interface ResolvedAsset {
  kind: 'content' | 'global' | 'theme'
  sourcePath: string
  assetPath: string
  publicPath: string
  absoluteUrl: string
...
// Content refs: compileAssets → contentPublicPaths[identity][ref] → publicPath
...
import type { ResolvedAsset } from '../../src/compiler/assets/types'
...
**Interfaces:**
- Consumes: `CompiledPage`, `RouteContentPackage`, `ReleaseOptions`, `selectReleaseArtwork`, Plan 04 asset resolvers
...
function releasePkg(partial: Partial<RouteContentPackage> & { slug: string; date: string }): RouteContentPackage {
  return {
...
import type { RouteContentPackage, ReleaseOptions } from '../../shared/types'
import type { CompiledPage, ContentIdentity } from '../../shared/route-types'
...
```bash
git add src/compiler/release/list-model.ts src/shared/release/types.ts tests/compiler/release/list-model.test.ts tests/helpers/release-fixtures.ts
...
```bash
npm install -D @vitejs/plugin-vue @vue/test-utils happy-dom
```
...
```bash
git add vitest.config.ts package.json package-lock.json src/client/components/DraftBadge.vue src/client/components/release/ReleaseArtwork.vue src/client/components/release/ReleaseIndexGrid.vue src/client/styles/release.css src/client/styles/index.ts tests/client/release/ReleaseIndexGrid.test.ts
...
**Files:**
- Create: `src/compiler/release/jsonld-album.ts`
- Create: `tests/compiler/release/jsonld-album.test.ts`
...
```ts
// tests/compiler/release/jsonld-album.test.ts
import { describe, expect, it } from 'vitest'
import { buildAlbumJsonLd } from '../../../src/compiler/release/jsonld-album'
...
Run: `npm test -- tests/compiler/release/jsonld-album.test.ts`
...
```ts
// src/compiler/release/jsonld-album.ts
...
```bash
git add src/compiler/release/jsonld-album.ts tests/compiler/release/jsonld-album.test.ts
...
**Interfaces:**
- Consumes: `CompiledPage`, `RouteContentPackage`, `Book`, image roles, `numberDiscs`, `buildAlbumJsonLd`, asset resolvers
...
- Modify: `src/compiler/release/index.ts` (barrel exporting list/detail/jsonld builders)
...
Run: `npm test -- tests/compiler/release/acceptance.test.ts`

Expected: FAIL with module not found for `src/compiler/release/index.ts` and/or `releaseCollectionPage`
...
```ts
// src/compiler/release/index.ts — already shown in Step 1; create the file if missing
```

Re-export Release builders from the theme Node entry used by Plan 05’s app enhancer (`src/compiler/index.ts` or equivalent already present from prior plans):
...
```bash
git add src/compiler/release/index.ts src/index.ts tests/compiler/release/acceptance.test.ts tests/helpers/release-fixtures.ts
...
**Deferred (intentionally out of this plan):** News/Page UI (Plan 09), full SEO/OG/`hreflang`/RSS/Sitemap emission (Plan 10 — Album JSON-LD graph is produced here for Release to consume), npm package publish (Plan 11), shell/platform/asset internals (Plans 04–07).

## Downstream note (Plan 04 contract sync)

When executing this plan against revised Plan 04:

- Import `ResolvedAsset` from `src/shared/asset-types` (not `src/compiler/assets/types` or `src/node/assets/types`).
- Use `publicPath` (VuePress-base-prefixed) everywhere this plan historically said `publicUrl`.
- Prefer `AssetManifest.contentPublicPaths` / client `resolveContentAsset` for package refs; `resolveGlobalAsset` takes the Plan 04 input object (configDir/base/siteUrl), not a bare relative string.
- Fixture helper `asset(publicUrl, …)` should map the first argument onto `publicPath` (and derive `assetPath`/`absoluteUrl`/`contentHash` as needed for tests).
