...
- Plan 05's `LOCALE_STORAGE_KEY` must equal the root router's value `synctrol:locale`. Because client code must not import `src/compiler/**`, Plan 05 Task 1 moves the canonical constant to `src/shared/locale-storage.ts`; `src/compiler/root-router-html.ts` imports (and may re-export) from shared. LanguageSwitcher imports from shared. Locale-home hrefs must use the same encoded `publicPath` convention as the root router (`encodePathSegment` / compiled `publicPath`), not raw CJK keys.
