...
describe('ShellLayout', () => {
  ...
  it('renders the content container as a section and the language switcher inside the footer', () => {
    ...
    // Social links live in the nav side-panel (desktop + drawer), not the footer
    expect(footer.find('.syn-social-links').exists()).toBe(false)
    expect(wrapper.findAll('.syn-language')).toHaveLength(1)
    expect(wrapper.findAll('.syn-social-links')).toHaveLength(2)
  })
})
