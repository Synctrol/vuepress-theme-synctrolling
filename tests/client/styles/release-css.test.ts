...
describe('release.css', () => {
  ...
  it('defines standalone platform entries that fill the row', () => {
    ...
    expect(css).toMatch(/\.syn-platform-iframe\[data-embed-kind='video'\]\s*\{[^}]*aspect-ratio:\s*4\s*\/\s*3/)
    expect(css).toMatch(/\.syn-platform-iframe\[data-embed-kind='player'\]\s*\{[^}]*aspect-ratio:\s*2\s*\/\s*1/)
    expect(css).toMatch(/\.syn-platform-iframe\[data-embed-kind='fixed-height'\]\s*\{[^}]*height:\s*450px/)
    expect(css).not.toMatch(/\.syn-platform-iframe\[data-embed-kind='fixed-height'\]\s*\{[^}]*aspect-ratio:\s*4\s*\/\s*3/)
    const mobileIdx = css.indexOf('@media (max-width: 768px)')
    expect(mobileIdx).toBeGreaterThan(-1)
    const mobile = css.slice(mobileIdx)
    expect(mobile).toMatch(/\.syn-platform-iframe\[data-embed-kind='player'\]\s*\{[^}]*aspect-ratio:\s*1\s*\/\s*1/)
    expect(mobile).not.toMatch(/\.syn-platform-iframe\[data-embed-kind='video'\]\s*\{[^}]*aspect-ratio/)
    ...
    expect(css).toMatch(/\.syn-platform-link\s*\{[^}]*border:\s*var\(--syn-border-strong\)/)
    expect(css).toMatch(/\.syn-platform-link\s*\{[^}]*text-transform:\s*uppercase/)
    expect(css).toMatch(/\.syn-platform-link:hover[^{]*\{[^}]*background:\s*var\(--syn-fg\)/)
  })
