...
describe('shell.css', () => {
  ...
  it('renders the navigation column as a deco link-panel column', () => {
    const css = readFileSync(resolve('src/client/styles/shell.css'), 'utf8')
    expect(css).toMatch(/\.syn-navigation__side-panel\s*\{[^}]*flex:\s*1/)
    expect(css).toMatch(/\.syn-navigation__side-panel\s*\{[^}]*box-shadow:\s*inset\s*0\s*3px\s*0\s*currentColor/)
    expect(css).toMatch(/\.syn-navigation__side-panel\s*\{[^}]*padding:\s*0 24px/)
    expect(css).toMatch(/\.syn-navigation__list\s*\{[^}]*flex:\s*1\.618/)
    ...
    expect(css).toMatch(/\.syn-navigation\s*\{[^}]*flex-direction:\s*column/)
    expect(css).toMatch(/\.syn-navigation__list\s*\{[^}]*flex:\s*1/)
