...
describe('shell.css', () => {
  ...
  it('defines the general Box, Notice, Accordion, Divider and Figure components', () => {
    ...
    expect(css).toMatch(/\.syn-new-album\s*\{[^}]*max-width:\s*400px/)
    expect(css).toMatch(/\.syn-new-album\s*\{[^}]*margin-inline-start:\s*auto/)
    expect(css).toMatch(/\.syn-new-album\s*\{[^}]*background-size:\s*contain/)
    expect(css).toMatch(/\.syn-new-album\s*\{[^}]*background-repeat:\s*no-repeat/)
