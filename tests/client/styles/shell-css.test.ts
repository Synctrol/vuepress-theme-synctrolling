...
describe('shell.css', () => {
  ...
  it('styles the home logo with reference .logo/.logo-sub classes and full-width title cell', () => {
    ...
    expect(css).toMatch(/\.syn-main\s*\{[^}]*padding:\s*24px 16px/)
  ...
  it('defines the general Box, Notice, Accordion, Divider and Figure components', () => {
    ...
    expect(css).toMatch(/\.syn-new-album\s*\{[^}]*width:\s*100%/)
    ...
    expect(css).toMatch(/\.syn-new-album::before\s*\{[^}]*background-size:\s*100%\s+100%,\s*contain/)
