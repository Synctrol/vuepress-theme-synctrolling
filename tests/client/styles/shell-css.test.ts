...
describe('shell.css', () => {
  ...
  it('defines the general Box, Notice, Accordion, Divider and Figure components', () => {
    ...
    expect(css).toMatch(/\.syn-new-album\s*\{[^}]*margin-block-end:\s*36px/)
    ...
    expect(css).toMatch(/\.syn-new-album::before\s*\{[^}]*width:\s*min\(460px,\s*100%\)/)
    expect(css).toMatch(/\.syn-new-album::before\s*\{[^}]*inset-inline-end:\s*0/)
    expect(css).toMatch(/\.syn-new-album::before\s*\{[^}]*z-index:\s*-1/)
    expect(css).toMatch(/\.syn-new-album::before\s*\{[^}]*background-image:\s*var\(--syn-new-album-image,\s*none\)/)
    expect(css).toMatch(/\.syn-new-album::before\s*\{[^}]*background-size:\s*cover/)
    expect(css).toMatch(/\.syn-new-album::before\s*\{[^}]*background-position:\s*var\(--syn-new-album-position,\s*right\s+center\)/)
    expect(css).toMatch(/\.syn-new-album\s*\{[^}]*padding:\s*1rem\s+1\.25rem\s+calc\(1rem\s*\+\s*1\.5em\)/)
    expect(css).toMatch(/\.syn-new-album\s*\{[^}]*line-height:\s*1\.5/)
    expect(css).not.toMatch(/\.syn-new-album\s*\{[^}]*min-height:\s*120px/)
    expect(css).toMatch(/\.syn-new-album__body\s*\{[^}]*width:\s*min\(400px,\s*100%\)/)
    expect(css).toMatch(/\.syn-new-album__body\s*\{[^}]*margin-inline-start:\s*auto/)
    expect(css).toMatch(/\.syn-new-album::after\s*\{[^}]*inset:\s*0/)
    expect(css).toMatch(/\.syn-new-album::after\s*\{[^}]*background:\s*var\(--syn-new-album-scrim\)/)
    expect(css).toMatch(/\.syn-new-album::after\s*\{[^}]*opacity:\s*var\(--syn-new-album-scrim-opacity\)/)
    expect(css).toMatch(/\.syn-new-album::after\s*\{[^}]*transition:\s*opacity\s*0\.2s/)
    expect(css).toMatch(/\.syn-new-album\s*\{[^}]*--syn-new-album-scrim:\s*#000/)
    expect(css).toMatch(/\.syn-new-album\s*\{[^}]*--syn-new-album-scrim-opacity:\s*0\.25/)
    expect(css).toMatch(/\.syn-new-album:hover[^{]*\{[^}]*--syn-new-album-scrim-opacity:\s*0\.45/)
    expect(css).toMatch(/:root\[data-theme='dark'\]\s*\.syn-new-album\s*\{[^}]*--syn-new-album-scrim:\s*#fff/)
