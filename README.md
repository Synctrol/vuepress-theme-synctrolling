...
首页必须有 `home-logo` 区块，而且全站只能有一个首页。`<NewAlbumReleased />` 可选：像插在 Logo 下方的一整块可点击横幅（无边框、右对齐、最宽 400px），展示专辑名与介绍文案，标题右侧带 ↗ 箭头；`background` 指向首页内容包里的图片（如 `./assets/album.svg`）作为背景（右对齐、不裁切，图片以外的区域只显示遮罩），`position` 可选指定背景重心（默认 `right center`），`href` 可选（提供后整块都是链接）。遮罩与文字颜色随主题模式自动翻转：亮色为深色遮罩配白字，暗色为浅色遮罩配黑字。
